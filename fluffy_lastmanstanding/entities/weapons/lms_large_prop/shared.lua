SWEP.PrintName			= "Large Prop Propultion Unit" -- This will be shown in the spawn menu, and in the weapon selection menu
SWEP.Author			= "Daniel Hawkins" -- These two options will be shown when you have the weapon highlighted in the weapon selection menu
SWEP.Instructions		= "Left mouse to lob a huge prop"
...
SWEP.ShootSound = Sound( "Weapon_Mortar.Single" )

local MaxMass = 200

local Props = {
	"models/props_c17/canister02a.mdl", --Too heavy for small weapon gun
...
function SWEP:PrimaryAttack()
	--Fire a melon every second
	self:SetNextPrimaryFire( CurTime() + 100)	
	local PropToUse = table.Random(Props)
	-- Call 'ThrowChair' on self with this model
	self:ThrowChair( PropToUse )
end
...
function SWEP:ThrowChair( model_file )
	...
	--Dont let any of our big props be heavier than our max mass
	if phys:GetMass() > MaxMass then
		phys:SetMass(MaxMass)
 	end
	...
	aimvec:Mul( math.random(75000, 100000) )
